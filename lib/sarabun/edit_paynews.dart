...
class editpaynews extends StatefulWidget {
    final paynewsModel newpayModels;
  const editpaynews({Key? key, required this.newpayModels}) : super(key: key);

  @override
  State<editpaynews> createState() => _editpaynewsState();
}

class _editpaynewsState extends State<editpaynews> {
   paynewsModel? newpayModels;
  ...
  void initState() {
    ...
    super.initState();
    newpayModels = widget.newpayModels;

    titleController.text = newpayModels!.book_name;
    // print('### image from mySQL ==>> ${productModel!.images}');
  }
  Widget build(BuildContext context) {
    return Scaffold(
appBar: AppBar(title: Text('แก้ไขข้อมูลจัดซื้อจัดจ้าง'),),
   body: LayoutBuilder(
    ...
    if (formKey.currentState!.validate()) {
      ...
      String id = newpayModels!.book_key;

      String apiEditProduct =
          '${MyConstant.domain}/dopa/api/edit_newspay.php?isUpdate=true&id=$id&remark=$titel';
  ...
  Future<Null> processdel() async {
    ...
    String apiEditProduct =
        '${MyConstant.domain}/dopa/api/del_newspay.php?isDelete=true&id=$id';
